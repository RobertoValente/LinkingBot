...
#define pin360 A0 //AMARELO-DIV / VERMELHO-5V / CASTANHO-GND
Servo servo360;
/* Servos --------------------------------------------------------------------*/
...
const float z_default = -80, z_up = -30, z_boot = z_absolute; //z_default = -50
const float z_dance = -50;
...
const float leg_move_speed = 22; //DEFAULT: 8 //ANTES:16
const float body_move_speed = 6; //3
const float stand_seat_speed = 1;
volatile int rest_counter;      //+1/0.02s, for automatic rest
//functions' parameter
const float KEEP = 255; //255
...
#define PIN A1 //LARANJA-DIV / PRETO-GND / VERDE-5V
#define NUMPIXELS 2
...
/*
  ...
  0 - volta completa - horário
  90 - fica parado
  180 - volta completa - anti-horário
*/  


// O LADO DA FRENTE É O QUE NÃO TEM O BOTÃO!

void mexerCabecaCrazy() {
  servo[4][0].attach(servo_pin[4][0]);
  servo[4][0].write(0);
  delay(1000);
  servo[4][0].detach();
  delay(1000);
...
void ligarLedsFade(int r, int g, int b) {
  for(int i = 0; i < NUMPIXELS; i++) {
    pixels.setPixelColor(i, pixels.Color(r, g, b));
    pixels.show();
  }
...
void rodarCabeca() {
  // Botão para trás: Esquerda
  servo360.attach(pin360);
  servo360.write(180);
  delay(1000);
  servo360.detach();
  delay(1000);
...
void loop()
{
  // DELAY DA ARANHA NA PARTE INICIAL
  delay(84000);

  currentMillis = millis();
  Serial.print(currentMillis); //1205ms
  Serial.print("====================");
  stand();
  delay(1000);
  ligarLedsFade(255, 255, 255); //2seg
  rodarCabeca(); //4seg
  delay(500);
  sit();
  //apagarLeds();
  ...
  step_forward(10);
  //currentMillis = millis();
  //Serial.print(currentMillis); //23093ms
  //Serial.print("====================");

  //meter delay

  step_back(2); //38seg com 5
  delay(500);

  ligarLedsFade(255, 0, 0); //2seg
  rodarCabeca(); //4seg
  //apagarLeds();
  ledWhiteBlue();
...
void body_dance(int i)
{
  ...
  wait_all_reach();
  //stand();
  set_site(0, x_default, y_default, z_dance - 20);
  set_site(1, x_default, y_default, z_dance - 20);
  set_site(2, x_default, y_default, z_dance - 20);
  set_site(3, x_default, y_default, z_dance - 20);
  wait_all_reach();
